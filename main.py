...
pygame.init()
pygame.mixer.init()
screen = pygame.display.set_mode((1280, 1080)) #720
clock = pygame.time.Clock()
pygame.display.set_caption("Dinosour Game")

game_font = pygame.font.Font("PressStart2P-Regular.ttf", 25)
...
jump_sfx.set_volume(1.0)  # Max volume
points_sfx.set_volume(1.0)
...
while True:
    ...
    if pygame.sprite.spritecollide(dino_group.sprite, obstacle_group, False):
        if dinosaur.rect.centery >= 360:  # Check if the dinosaur is on the ground
         game_over = True
         death_sfx.play()
    if game_over:
        end_game()
